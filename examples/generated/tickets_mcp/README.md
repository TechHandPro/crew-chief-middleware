...
MCP tools for **Example Tickets API** (1.4.0), generated from an OpenAPI document by
openapi-to-mcp 0.1.0 (crew-chief-middleware).

- Base URL: `https://tickets.example.com/api/v1`
...
| --- | --- |
| `TICKETS_API_TOKEN` | bearer token (or a pre-obtained OAuth2 access token) |
| `TICKETS_BASE_URL` | override the API base URL |
| `TICKETS_READ_ONLY` | expose only non-mutating tools |
| `TICKETS_TIMEOUT_SECONDS` | per-request timeout (default 30) |
| `TICKETS_MAX_RESPONSE_BYTES` | response cap before truncation (default 100000) |
| `TICKETS_MAX_RETRIES` | retries for idempotent requests on 429/502/503/504 (default 2) |
| `TICKETS_ALLOW_INSECURE_HTTP` | allow plaintext http to a non-loopback host |
| `TICKETS_LOG_LEVEL` | stderr log level (default WARNING) |

## Tools

| Tool | Method | Path | Read-only |
| --- | --- | --- | --- |
| `list_tickets` | GET | `/tickets` | yes |
| `create_ticket` | POST | `/tickets` | no |
| `get_ticket` | GET | `/tickets/{ticketId}` | yes |
| `delete_ticket` | DELETE | `/tickets/{ticketId}` | no |
| `update_ticket` | PATCH | `/tickets/{ticketId}` | no |
| `list_ticket_comments` | GET | `/tickets/{ticketId}/comments` | yes |
| `add_ticket_comment` | POST | `/tickets/{ticketId}/comments` | no |
